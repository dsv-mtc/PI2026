...
- Para cada colegio:
    ...
    - Popup con descripción, código CE, UBIGEO y Lat/Long
- Siniestros fatales: puntos rojos (solo si caen dentro del contorno)
- Leyenda: Azul (Nuevas) = COLOR_FALSE, Celeste (Mantenimiento) = COLOR_TRUE
- Contorno: provincial si termina en "01", si no distrital.
- UI derecha:
    - Buscador por nombre y código
    - Búsqueda por coordenadas (Long/Lat)
    - Medir distancia (dos puntos arrastrables) con etiqueta
    - Abrir vista en Google Maps
    - Botón para colapsar/expandir la caja de herramientas
"""
...
def build_popup_colegio(row: pd.Series) -> str:
    def fmt(name):
        v = row.get(name)
...
def map_for_excel(
    ...
    # ---------- UI derecha: buscadores + coords + medir + GMaps + Toggle ----------
    # SIN f-string: inyecciones por concatenación
    tpl = """
